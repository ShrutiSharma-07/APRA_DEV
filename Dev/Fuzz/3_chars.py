...
def main():
    ...
    def cleanse_title(title):
        ...
        # Remove articles (a, an, the) from beginning of title
    ...
    # Apply cleansing to both datasets
    print("Cleansing titles...")
    # Create copies of the original titles before cleansing
    apra_works['APRA_ORIGINAL_TITLE'] = apra_works['APRA_CLEANED_TITLE'].copy()
    muzooka_tracks['MUZOOKA_ORIGINAL_TITLE'] = muzooka_tracks['MUZOOKA_CLEANED_TITLE'].copy()

    # Now apply the cleansing
    apra_works['APRA_CLEANED_TITLE'] = apra_works['APRA_CLEANED_TITLE'].apply(cleanse_title)
    muzooka_tracks['MUZOOKA_CLEANED_TITLE'] = muzooka_tracks['MUZOOKA_CLEANED_TITLE'].apply(cleanse_title)

    # Create dictionary of muzooka tracks grouped by first 3 characters for faster matching
    print("Creating Muzooka lookup dictionary by first 3 characters...")
    muzooka_dict = {}
    for idx, row in muzooka_tracks.iterrows():
        title = row['MUZOOKA_CLEANED_TITLE']
        if len(title) >= 3:
            prefix = title[:3]
            if prefix not in muzooka_dict:
                muzooka_dict[prefix] = []
            muzooka_dict[prefix].append(row)

    def find_matches(row_apra_works, threshold):
        matches = []
        apra_title = row_apra_works['APRA_CLEANED_TITLE']
        ...
        # Only match against Muzooka tracks with same prefix
        if prefix in muzooka_dict:
            potential_matches = muzooka_dict[prefix]

            for row_m_tracks in potential_matches:
                score = fuzz.WRatio(apra_title, row_m_tracks['MUZOOKA_CLEANED_TITLE'])
                ...
                if score >= threshold:
                    ...
                    matches.append({
                        ...
                        "APRA_ORIGINAL_TITLE": row_apra_works['APRA_ORIGINAL_TITLE'],
                        "APRA_CLEANED_TITLE": apra_title,
                        "APRA_ISWC": row_apra_works['APRA_ISWC'],
                        "MUZOOKA_ORIGINAL_TITLE": row_m_tracks['MUZOOKA_ORIGINAL_TITLE'],
    ...
    for idx, row_apra_works in apra_works.iterrows():
        ...
        new_matches = find_matches(row_apra_works, threshold)
