...
# Process ADC titles and create MinHash signatures
...
# Function to match MZK titles with ADC titles
def match_mzk_titles(mzk_path, adc_minhash_df, threshold=0.8, chunk_size=10000, num_perm=NUM_PERM, sample_mode=False,
                     sample_size=10000):
    start_time = time.time()
    ...
    for _, mzk_row in track_progress(mzk_df.iterrows(), total=len(mzk_df), desc="Finding exact matches"):
        ...
        # Check for ISWC match first
        iswc_match = False
        if pd.notna(mzk_row['MUZOOKA_ISWC']) and mzk_row['MUZOOKA_ISWC'] != '':
            for _, adc_row in adc_minhash_df.iterrows():
                if pd.notna(adc_row['APRA_ISWC']) and adc_row['APRA_ISWC'] != '' and adc_row['APRA_ISWC'] == mzk_row[
                    'MUZOOKA_ISWC']:
                    iswc_match = True
        ...
        if mzk_title in adc_titles_dict:
            for adc_row in adc_titles_dict[mzk_title]:
                # Skip if we already found an ISWC match for this combo
                if iswc_match and pd.notna(adc_row['APRA_ISWC']) and pd.notna(mzk_row['MUZOOKA_ISWC']) and adc_row[
                    'APRA_ISWC'] == mzk_row['MUZOOKA_ISWC']:
                    continue
    ...
    for chunk_start in range(0, total_count, chunk_size):
        ...
        for idx, mzk_row in track_progress(chunk_df.iterrows(), total=len(chunk_df), desc="Finding fuzzy matches"):
            ...
            for candidate_idx in candidates:
                ...
                adc_row = adc_minhash_df.iloc[candidate_idx]

                # Calculate Jaro-Winkler similarity
                similarity = jaro_winkler.normalized_similarity(mzk_title, adc_row['NORMALIZED_TITLE'])

                # Check if ISWCs match
                iswc_match = False
                if pd.notna(mzk_row['MUZOOKA_ISWC']) and pd.notna(adc_row['APRA_ISWC']) and mzk_row[
                    'MUZOOKA_ISWC'] != '' and adc_row['APRA_ISWC'] != '':
                    iswc_match = (mzk_row['MUZOOKA_ISWC'] == adc_row['APRA_ISWC'])
    ...
    matches_path = os.path.join(OUTPUT_DIR, "title_matches_lsh.csv")
    ...
    if not matches_df.empty:
        ...
        iswc_matches = matches_df[matches_df['ISWC_MATCH'] == True]
        print(f"Matches with ISWC match: {len(iswc_matches)}")

        # Calculate stats for title matches
        title_matches = matches_df[matches_df['ISWC_MATCH'] == False]
...
def main():
    print("Starting LSH-based title matching process with local CSV files...")
    ...
    # Step 2: Match MZK titles against ADC titles
    print("\nStep 2: Matching MZK titles against ADC titles...")
