...
def create_enhanced_similarity_function(session):
    ...
    function parseNameComponents(name) {
        ...
        // Create standard format (ADC style: Last First Middle)
        ...
        // Create alternative format (Mazooka style: First Middle Last)
    ...
    function calculateMatchScore(adcFormat, mazookaFormat, adcScore, mazookaScore) {
        ...
        if (adcLast.toUpperCase() === mazookaLast.toUpperCase()) {
            lastNameScore = 2; // Exact match
        } else {
            // Check for fuzzy match
            const similarity = calculateStringSimilarity(adcLast, mazookaLast);
            if (similarity >= 0.93) {
        ...
        if (adcFirst && mazookaFirst) {
            if (adcFirst.toUpperCase() === mazookaFirst.toUpperCase()) {
                firstNameScore = 1; // Exact match
            } else {
                // Check for fuzzy match
                const similarity = calculateStringSimilarity(adcFirst, mazookaFirst);
                if (similarity >= 0.93) {
                    firstNameScore = 0.5; // Fuzzy match
                }
                // Only check initial match if we didn't get a better match already
                else if (firstNameScore === 0 && 
                        adcFirst.charAt(0).toUpperCase() === mazookaFirst.charAt(0).toUpperCase()) {
    ...
    function fixedNameSimilarityScore(adcName, mazookaName, adcScore, mazookaScore) {
        try {
            // Parse both names
            ...
            // 1. ADC standard vs Mazooka standard
            const score1 = calculateMatchScore(
                adcParsed.standard, 
                mazookaParsed.standard,
                adcScore, 
                mazookaScore
            );

            // 2. ADC standard vs Mazooka alternative
            const score2 = calculateMatchScore(
                adcParsed.standard, 
                mazookaParsed.alternative,
                adcScore, 
                mazookaScore
            );

            // 3. ADC alternative vs Mazooka standard
            const score3 = calculateMatchScore(
                adcParsed.alternative, 
                mazookaParsed.standard,
                adcScore, 
                mazookaScore
            );

            // 4. ADC alternative vs Mazooka alternative
            const score4 = calculateMatchScore(
                adcParsed.alternative, 
                mazookaParsed.alternative,
                adcScore, 
                mazookaScore
            ...
            // Find the best scoring format combination
            let bestScore = score1;
            if (score2.score > bestScore.score) bestScore = score2;
            if (score3.score > bestScore.score) bestScore = score3;
            if (score4.score > bestScore.score) bestScore = score4;
    ...
    return fixedNameSimilarityScore(ADC_NAME, MAZOOKA_NAME, APRA_SCORE, MAZOOKA_SCORE);
    $$
    """).collect()
